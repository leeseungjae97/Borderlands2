...
bool InputGeom::CreateGeom(Ptr<CMesh> _pMesh, Matrix _matWorld, Vec3 vScale)
{
	if (m_chunkyMesh)
	{
		delete m_chunkyMesh;
	...
	for (int i = 0; i < _pMesh->GetVtxCount(); ++i)
	{
		Vec3 vPos = vv[i].vPos;
		Vec3 vNormal = vv[i].vNormal;
		vPos = XMVector3TransformCoord(Vec3(vPos), _matWorld);
		m_verts.emplace_back(vPos.x);
		m_verts.emplace_back(vPos.y);
		m_verts.emplace_back(vPos.z);

		m_normals.emplace_back(vNormal.x);
		m_normals.emplace_back(vNormal.y);
		m_normals.emplace_back(vNormal.z);
	}

	vector<UINT> inds;
	UINT idx;
	for (auto indexInfo : _pMesh->GetIndexInfo())
	{
		inds.clear();
		...
		{
			idx = inds[idxIdx];
			m_tris.push_back(idx);
		}

	}

	m_vertCount = m_verts.size() / 3;
	m_triCount = m_tris.size() / 3;

	m_scale = vScale.x;

	rcCalcBounds(m_verts.data(), m_vertCount, m_meshBMin, m_meshBMax);
	...
	if (!rcCreateChunkyTriMesh(m_verts.data(), m_tris.data(), m_triCount, 256, m_chunkyMesh))
	{
		return false;
	}
