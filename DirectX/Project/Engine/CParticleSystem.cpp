	...
	, m_bFire(false)
{
	...
	m_SpawnCountBuffer->Create(sizeof(tRWParticleBuffer), 1, SB_TYPE::READ_WRITE, true);
...
void CParticleSystem::SetParticlePos(Vec3 _vPos)
{
	m_UpdateCS->SetParticleObjectPos(_vPos);
}

void CParticleSystem::SpawnParticleDT()
{
	// 스폰 레이트 계산
	float fTimePerCount = 1.f / (float)m_ModuleData.SpawnRate;
	m_AccTime += DT;
	// 한정된 생성 개수에 대한 생성 개수 판정
	// 0.01를 Acc넘기면
	if (fTimePerCount < m_AccTime)
	{
		// 초과 배율 ==> 생성 개수
		// DT가 1개당 스폰되는 시간을 초과하면 fData의 값이 커짐
		float fData = m_AccTime / fTimePerCount;

		// 나머지는 남은 시간
		// (생성 개수 - 초과분 반올림) -> 다음 생성까지 남은 시간
		// 개당 시간 * 다음 생성까지 남은 시간
		// 0.01 * 0.1 = 0.001
...
void CParticleSystem::finaltick()
{
	...
	m_UpdateCS->SetRWParticleBuffer(m_SpawnCountBuffer);
...
void CParticleSystem::render()
{
	...
	// Particle Render	
	Ptr<CTexture> pParticleTex = CResMgr::GetInst()->Load<CTexture>(L"texture\\particle\\Sparks.png", L"texture\\particle\\Sparks.png");
