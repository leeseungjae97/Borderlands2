...
//RWStructuredBuffer<tTileParticle> TileParticleBuffer : register(u2);
...
#define MousePos            g_vec4_1
//#define MousePos            g_vec3_0
...
[numthreads(1000, 1, 1)]
void CS_ParticleUpdate2(int3 _ID : SV_DispatchThreadID)
{
    ...
    float _FXX = _ID.x / 100.f;
    float _FYY = _ID.x % 100.f;

    float _FX = (ModuleData.vFixedScale.x * (_FXX - _FYY)) / 2.f;
    float _FY = (ModuleData.vFixedScale.y * (_FXX + _FYY)) / 2.f;

    particle.vWorldPos.xyz = particle.vLocalPos.xyz + ObjectPos.xyz;
    particle.vWorldPos.x += _FX;
    particle.vWorldPos.y += _FY;
    //particle.ScaleFactor = 1.f;
    
    particle.vWorldScale.xyz = ModuleData.vSpawnScaleMax.xyz;
    particle.vColor = float4(1.f, 0.f, 0.f, 1.f);

    float2 _MousePos = float2(MousePos.x, MousePos.y);
    float2 _Pos = particle.vWorldPos.xy;
    float2 _Scale = particle.vWorldScale.xy;
        
    if (PointIntersectRhombus(_Pos, _Scale, _MousePos) == true)
    {
        particle.ScaleFactor = ModuleData.MaxScaleFator;
    }else
    {
        if (particle.ScaleFactor >= ModuleData.MinScaleFator)
            particle.ScaleFactor -= 0.01f;
    }
    ...
    if (SpawnModule)
    {
        ...
        if (particle.Active == 0)
        {
            ...
            while (0 < SpawnCount)
            {
                ...
                int outvalue = 0;
                // dest spwan, comparev orgvalue, value spwan - 1, origin outvalue
                ...
                {   
                    ...
                    if (ModuleData.SpawnShapeType == 0)
                    {
                        ...
                        //particle.vLocalPos.x *= particle.vWorldScale.x;
                        //particle.vLocalPos.y *= particle.vWorldScale.x;
                        // 파티클 위치기준으로 원
                        particle.vWorldPos.xyz = particle.vLocalPos.xyz + ObjectPos.xyz;
                        //particle.vWorldPos.x += vOut2.r * 1000.f;
                        //particle.vWorldPos.y += vOut3.r * 1000.f;
