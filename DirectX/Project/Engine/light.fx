...
PS_OUT PS_DirLightShader(VS_OUT _in)
{
    ...
    else
    {
        ...
        float fLightDepth = ShadowMapTargetTex.Sample(g_sam_0, vShadowMapUV);
...
float4 PS_MergeShader(VS_OUT _in) : SV_Target
{
    ...
    //ShadowTargetTex.SampleCmp();
    //float fShadowPow = ShadowTargetTex.Sample(g_shadow_sampler, vScreenUV).r;
    //float fShadowPow = BlurDepth(ShadowTargetTex, g_shadow_sampler, float2(100.f, 100.f), vScreenUV).x;
    float fShadowPow = Blur2(ShadowTargetTex, g_sam_0, vScreenUV, float2(10.f, 10.f), 0.005f).x;
    //fShadowPow = Blur2(ShadowTargetTex, g_sam_0, vScreenUV, 0.0f, float2(1.f, 1.f));
