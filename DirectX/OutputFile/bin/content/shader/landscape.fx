...
#define ColorMap        g_tex_3
#define WeightMap       g_tex_4

#define TileCount       g_float_1
#define WeightMapResol  g_vec2_1
...
// 패치의 분할 횟수를 지정하는 함수
#define TessEdge    g_float_4
#define TessInside  g_float_5
    ...
    , uint PatchID : SV_PrimitiveID)
{
    ...
    //float fEdge = TessEdge;
    //float fInside = TessInside;
    //if (fEdge == 0.0f) 
    //    fEdge = 8.0f;
    ...
    output.Edges[0] = pow(2, (int) GetTessFactor(distance(vCamWorldPos, vUpDown), 1, 4, 500.f, 2000.f));
    output.Edges[1] = pow(2, (int) GetTessFactor(distance(vCamWorldPos, vLeftRight), 1, 4, 500.f, 2000.f));
    output.Edges[2] = pow(2, (int) GetTessFactor(distance(vCamWorldPos, vSlide), 1, 4, 500.f, 2000.f));
    output.Inside = pow(2, (int) GetTessFactor(distance(vCamWorldPos, vMid), 1, 4, 500.f, 2000.f));
...
struct DS_OUT
{
    ...
    float2 vUV : TEXCOORD;
    float2 vHeightMapUV : TEXCOORD1;
    float4 vColorMapColor : COLOR;
...
PS_OUT PS_LandScape(DS_OUT _in)
{
    ...
    if (g_btexarr_0)
    {
        ...
        // 타일 색상
        int2 iWeightIdx = (int2) (_in.vHeightMapUV * WeightMapResol);
        float4 vWeight = WEIGHT_MAP[iWeightIdx.y * (int) WeightMapResol.x + iWeightIdx.x];
